...
class ClimateDataset(Dataset):
    def __init__(self, inputs_norm_dask, outputs_dask, output_is_normalized=True, sequence_length=3):
        # Store dataset size
        self.size = inputs_norm_dask.shape[0] # Total raw time steps
    ...
    def __getitem__(self, idx):
        ...
        current_slice = self.input_tensors[max(0, start_idx) : end_idx]

        
        num_missing_elements = self.sequence_length - len(current_slice)

        if num_missing_elements > 0:
            padding_tensor = current_slice[0].unsqueeze(0).repeat(num_missing_elements, 1, 1, 1)
            input_sequence = torch.cat((padding_tensor, current_slice), dim=0)
        else:
            
            input_sequence = current_slice
...
class ClimateEmulationDataModule(LightningDataModule):
    ...
    def setup(self, stage: str | None = None):
        ...
        with xr.open_zarr(self.hparams.path, consolidated=True, chunks={"time": 24}) as ds:
            ...
            val_input_dask_list, val_output_dask_list = [], []
            val_ssp = "ssp370"
            val_months = 120
            ...
            for i in range(2):
                for ssp in self.hparams.train_ssps:
                    ...
                    if ssp == val_ssp:
                        ...
                        val_input_dask_list.append(ssp_input_dask[-val_months:])
                        val_output_dask_list.append(ssp_output_dask[-val_months:])
            ...
            val_input_dask = da.concatenate(val_input_dask_list, axis=0)
            val_output_dask = da.concatenate(val_output_dask_list, axis=0)
